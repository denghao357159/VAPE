...
public class NewMoveCB{
	...
	public NewWindow NewMoveCB(Context ct, String p1, boolean chongb, Object[] onClick) {
		return null;
	}
    ...
    public PopupWindow NewMoveCB(Context ct,Object text,final boolean mod,final Object[] runs,int MoveX,int MoveY){
        ...
        _nmcbll2.setGravity(Gravity.CENTER);
		...
		_nmcbll2.setOnTouchListener(new View.OnTouchListener(){
				...
				public boolean onTouch(View view, MotionEvent event) {
					...
					if (action == MotionEvent.ACTION_DOWN) {
                        ...
                        //松开  
						if (!canmovew){
                            ((OnClickListener)(runs[0])).onClick(view);
                            if((boolean)(runs[1])==true){
                                if(QH==false){
                                    QH=true;
                                    _nmcbll2.setBackgroundDrawable(ASUI.roundBG(new String[]{"#DAFFCE","#BBFFEA","LR"},15,null,null));
						...
						canmovew = false;
					} else if (action == MotionEvent.ACTION_MOVE && canmovew) {
                        //移动
						nowX = event.getRawX();
                        nowY = event.getRawY();
                        // 计算XY坐标偏移量
